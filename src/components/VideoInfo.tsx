            ...
            {videoInfo && (
                <CardContent className='p-2 '>
                    <div className='flex items-center p-2 gap-2 w-full rounded-md relative hover:bg-slate-500 transition-colors duration-300 ease-in-out'>
                        <span className='w-10'>{playingVideo ? <FaPlay size="16px" /> : index + 1}</span>
                        <div className='flex items-center flex-col md:flex-row md:w-[400px] gap-4 cursor-pointer' onClick={() => { changeVideo(id) }}>
                            <img src={videoInfo?.thumbnails?.default?.url} alt="Thumbnail" />
                            <h5 className='space-x-5'>{videoInfo?.title?.split(' ').slice(0, 10).join(' ')}{videoInfo?.title?.split(' ').length > 10 ? '...' : ''}</h5>
                        </div>
